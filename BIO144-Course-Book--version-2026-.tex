...
\begin{tcolorbox}[enhanced jigsaw, leftrule=.75mm, colback=white, title=\textcolor{quarto-callout-warning-color}{\faExclamationTriangle}\hspace{0.5em}{Warning}, breakable, rightrule=.15mm, colframe=quarto-callout-warning-color-frame, opacitybacktitle=0.6, coltitle=black, colbacktitle=quarto-callout-warning-color!10!white, opacityback=0, left=2mm, bottomrule=.15mm, bottomtitle=1mm, toptitle=1mm, arc=.35mm, titlerule=0mm, toprule=.15mm]
...
\begin{tcolorbox}[enhanced jigsaw, leftrule=.75mm, colback=white, title=\textcolor{quarto-callout-important-color}{\faExclamation}\hspace{0.5em}{Important}, breakable, rightrule=.15mm, colframe=quarto-callout-important-color-frame, opacitybacktitle=0.6, coltitle=black, colbacktitle=quarto-callout-important-color!10!white, opacityback=0, left=2mm, bottomrule=.15mm, bottomtitle=1mm, toptitle=1mm, arc=.35mm, titlerule=0mm, toprule=.15mm]
...
\textbf{Think--Pair--Share} (\#a\_r\_vs\_rstudio\_roles) What is one
thing that R does, and one thing that RStudio does? Why is it useful
...
\textbf{Think--Pair--Share} (\#a\_objects\_not\_files) When you type
\texttt{a\ \textless{}-\ ...}, where does \texttt{x} live?
...
\textbf{Think--Pair--Share} (\#a\_errors\_are\_information) When we do
something that R does not understand, it often gives us an error
message. In red! What will you do when you get an error message?
...
\begin{tcolorbox}[enhanced jigsaw, leftrule=.75mm, colback=white, title=\textcolor{quarto-callout-tip-color}{\faLightbulb}\hspace{0.5em}{Tip}, breakable, rightrule=.15mm, colframe=quarto-callout-tip-color-frame, opacitybacktitle=0.6, coltitle=black, colbacktitle=quarto-callout-tip-color!10!white, opacityback=0, left=2mm, bottomrule=.15mm, bottomtitle=1mm, toptitle=1mm, arc=.35mm, titlerule=0mm, toprule=.15mm]
...
\begin{tcolorbox}[enhanced jigsaw, leftrule=.75mm, colback=white, title=\textcolor{quarto-callout-note-color}{\faInfo}\hspace{0.5em}{Note}, breakable, rightrule=.15mm, colframe=quarto-callout-note-color-frame, opacitybacktitle=0.6, coltitle=black, colbacktitle=quarto-callout-note-color!10!white, opacityback=0, left=2mm, bottomrule=.15mm, bottomtitle=1mm, toptitle=1mm, arc=.35mm, titlerule=0mm, toprule=.15mm]
...
\textbf{Think-Pair-Share} (\#tps-guess-params) Make a guess of the
slope. Make a guess of the intercept (hint be careful, lots of people
get this wrong).
...
\textbf{Think--Pair--Share} (\#a\_interpreting\_slope) In words, what
does the slope represent in this model? What does it \emph{not} tell
you?
...
\textbf{Think-Pair-Share} (\#tps-what-units)
...
\FunctionTok{ggplot}\NormalTok{(bp\_age\_data, }\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ fitted, }\AttributeTok{y =}\NormalTok{ sqrt\_abs\_R\_i)) }\SpecialCharTok{+}
  \FunctionTok{geom\_point}\NormalTok{() }\SpecialCharTok{+}
  \FunctionTok{labs}\NormalTok{(}\AttributeTok{x =} \StringTok{"Fitted values"}\NormalTok{, }\AttributeTok{y =} \FunctionTok{expression}\NormalTok{(}\FunctionTok{sqrt}\NormalTok{(}\FunctionTok{abs}\NormalTok{(R[i])))) }\SpecialCharTok{+}
  \FunctionTok{theme\_minimal}\NormalTok{()}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{3.1-regression-part1_files/figure-pdf/unnamed-chunk-27-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{3.1-regression-part1_files/figure-pdf/unnamed-chunk-33-1.pdf}}
...
\textbf{Think-Pair-Share} (\#tps-odd-data) Where on this graph would you
expect to see particularly influential outliers? Influential in the
sense that they would have a large effect on the slope of the line of
best fit.
...
\textbf{Think-Pair-Share} (\#tps-like-weight) What quantity that we
already experienced is like the weight of the child?
...
\pandocbounded{\includegraphics[keepaspectratio]{3.1-regression-part1_files/figure-pdf/unnamed-chunk-40-1.pdf}}
...
\textbf{Think--Pair--Share} (\#a\_linear\_model\_assumptions) Which
assumption of linear regression feels most fragile in real biological
data? Why?
...
\NormalTok{p1 }\OtherTok{\textless{}{-}} \FunctionTok{ggplot}\NormalTok{(eg\_data, }\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ x, }\AttributeTok{y =}\NormalTok{ y)) }\SpecialCharTok{+}
  \FunctionTok{geom\_point}\NormalTok{() }\SpecialCharTok{+} 
  \FunctionTok{theme\_bw}\NormalTok{() }\SpecialCharTok{+} \FunctionTok{geom\_smooth}\NormalTok{(}\AttributeTok{formula =}\NormalTok{ y }\SpecialCharTok{\textasciitilde{}}\NormalTok{ x, }\AttributeTok{method =} \StringTok{"lm"}\NormalTok{, }\AttributeTok{se =} \ConstantTok{FALSE}\NormalTok{)}
...
\pandocbounded{\includegraphics[keepaspectratio]{3.1-regression-part1_files/figure-pdf/unnamed-chunk-48-1.pdf}}
...
\begin{center}
\includegraphics[width=14cm,height=\textheight,keepaspectratio]{3.1-regression-part1_files/figure-pdf/unnamed-chunk-50-1.pdf}
\end{center}
...
\textbf{Think-Pair-Share} (\#tps-better-model) In which of these two
would you say the model is better, and in which is it worse?
...
\textbf{Think-Pair-Share} (\#tps-what-minimised) What is minimised when
we fit a regression model? And therefore what is maximised?
...
\textbf{Think-Pair-Share} (\#tps-null-hypothesis) Write down the null
hypothesis of no relationship between \(x\) and \(y\) in terms of a
\(\beta\) parameter.
...
\textbf{Think-Pair-Share} (\#tps-chance-area) In what areas of the graph
is the slope estimate more likely to have been observed by chance? And
what regions is it less likely to have been observed by chance? Think
about this before you look at the end of this chapter for an answer
(Section \textbf{?@sec-visual-p-values-regress}).
...
\textbf{Think--Pair--Share} (\#a\_prediction\_vs\_estimation) Which
interval answers: ``What is the mean response?'' Which answers: ``What
might a new observation look like?''
...
\FunctionTok{ggplot}\NormalTok{(}\FunctionTok{data.frame}\NormalTok{(}\AttributeTok{slopes =}\NormalTok{ rand\_slopes), }\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ slopes)) }\SpecialCharTok{+}
  ...
  \FunctionTok{geom\_vline}\NormalTok{(}\AttributeTok{xintercept =} \FunctionTok{coef}\NormalTok{(m)[}\DecValTok{2}\NormalTok{], }\AttributeTok{color =} \StringTok{"red"}\NormalTok{) }\SpecialCharTok{+}
  \FunctionTok{theme\_minimal}\NormalTok{()}
\end{Highlighting}
\end{Shaded}
...
\FunctionTok{ggplot}\NormalTok{(bp\_data\_diet, }\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ diet, }\AttributeTok{y =}\NormalTok{ bp)) }\SpecialCharTok{+}
  ...
  \FunctionTok{labs}\NormalTok{(}\AttributeTok{title =} \StringTok{"Blood Pressure by Diet Type"}\NormalTok{,}
       ...
       \AttributeTok{y =} \StringTok{"Blood Pressure (mm Hg)"}\NormalTok{) }\SpecialCharTok{+}
  \FunctionTok{theme\_minimal}\NormalTok{()}
\end{Highlighting}
\end{Shaded}
...
\textbf{Think--Pair--Share (\#a\_what\_f\_test\_answers)} What question
does the ANOVA F-test answer? What question does it \emph{not} answer?
...
\textbf{Think--Pair--Share} (\#a\_anova\_is\_regression) What changes
when we move from regression to ANOVA? What stays exactly the same?
...
\textbf{Think-Pair-Share} (\#tps-two-shape) Graphically, a linear
regression with one explanatory variable is a line. What is a geometric
representation of linear regression with two explanatory variables?
...
\textbf{Think-Pair-Share} (\#tps-assump-match) Review how we can check
these assumptions in the simple linear regression model. Match the
following to the assumptions above:
...
Residuals:
     Min       1Q   Median       3Q      Max 
-22.2857  -7.1690   0.7716   6.1979  29.8264 

Coefficients:
              Estimate Std. Error t value Pr(>|t|)    
(Intercept)   91.26506    3.41879  26.695  < 2e-16 ***
age            0.48673    0.05337   9.119 1.07e-14 ***
mins_exercise -0.11103    0.01078 -10.297  < 2e-16 ***
---
Signif. codes:  0 '***' 0.001 '**' 0.01 '*' 0.05 '.' 0.1 ' ' 1

Residual standard error: 9.727 on 97 degrees of freedom
Multiple R-squared:  0.6931,    Adjusted R-squared:  0.6868 
F-statistic: 109.5 on 2 and 97 DF,  p-value: < 2.2e-16
\end{verbatim}

In the final line of output we see ``F-statistic: 109.53 on 2 and 97 DF,
p-value: \ensuremath{1.32\times 10^{-25}}''.
...
associated with blood pressure (F(2, 97) = 109.53, p =
\ensuremath{1.32\times 10^{-25}}).'' Note that this is rather an
...
\begin{verbatim}
                Estimate Std. Error    t value     Pr(>|t|)
(Intercept)   91.2650569 3.41878667  26.695160 1.741966e-46
age            0.4867250 0.05337211   9.119464 1.068517e-14
mins_exercise -0.1110277 0.01078227 -10.297241 3.061971e-17
\end{verbatim}
...
\begin{verbatim}
                   2.5 %      97.5 %
(Intercept)   84.4797118 98.05040208
age            0.3807962  0.59265391
mins_exercise -0.1324275 -0.08962783
\end{verbatim}
...
\textbf{Think--Pair--Share (\#a\_partial\_effects)} What does ``holding
other variables constant'' mean here? Is this always biologically
plausible?
...
\textbf{Think-Pair-Share} (\#tps-variable-importance) How might we
assess how important is each of the explanatory variables, and how
important they are relative to each other?
...
\textbf{Think-Pair-Share} (\#tps-exercise-age) How many minutes of
exercise per week would we need to add to our fitness schedule to
compensate for the blood pressure increase associated with one year of
age?
...
\textbf{Think-Pair-Share} (\#tps-collinearity1) Imagine if there was
perfect correlation between age and minutes of exercise. This would mean
that a graph of age vs minutes of exercise would show a perfect line.
What would be the implications for the multiple linear regression model?
For example, what would be the \(R^2\) value for a model with one
...
\textbf{Think--Pair--Share} (\#a\_collinearity\_intuition) Why can
strong correlation among predictors make estimates unstable even if the
...
(\#a\_interpreting\_coefficients\_conditionally) How does the
interpretation of a coefficient change when you add another predictor to
the model?
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{plotly}\SpecialCharTok{::}\FunctionTok{plot\_ly}\NormalTok{(bp\_data\_multreg, }\AttributeTok{x =} \SpecialCharTok{\textasciitilde{}}\NormalTok{age, }\AttributeTok{y =} \SpecialCharTok{\textasciitilde{}}\NormalTok{mins\_exercise, }\AttributeTok{z =} \SpecialCharTok{\textasciitilde{}}\NormalTok{bp, }\AttributeTok{type =} \StringTok{"scatter3d"}\NormalTok{, }\AttributeTok{mode =} \StringTok{"markers"}\NormalTok{, }\AttributeTok{marker =} \FunctionTok{list}\NormalTok{(}\AttributeTok{size =} \DecValTok{5}\NormalTok{))}
\end{Highlighting}
\end{Shaded}
...
\bottomrule\noalign{}
\endlastfoot
(Intercept) & 91.265 & 3.419 & 26.695 & 0 & 84.480 & 98.050 \\
age & 0.487 & 0.053 & 9.119 & 0 & 0.381 & 0.593 \\
mins\_exercise & -0.111 & 0.011 & -10.297 & 0 & -0.132 & -0.090 \\
\end{longtable}
...
age & 0.49 & 0.38, 0.59 & <0.001 \\ 
mins\_exercise & -0.11 & -0.13, -0.09 & <0.001 \\ 
\bottomrule
\end{tabular*}
...
\textbf{Think-Pair-Share} (\#tps-general-diet) Can we say anything
general about the effect of diet on blood pressure?

No, we can't. We cannot, for example, state that a vegetarian diet
lowers blood pressure. We can say, however, that a vegetarian diet
lowers blood pressure of people that do little exercise.

\textbf{Think-Pair-Share} (\#tps-general-exercise) Can we say anything
general about the effect of exercise on blood pressure?

Well, we can say that exercise lowers blood pressure, but we have to be
careful. We have to say that exercise lowers blood pressure of people
with a meat heavy diet more than it lowers blood pressure of people with
a vegetarian diet.
...
\textbf{Think--Pair--Share} (\#a\_when\_lines\_cross) What does it mean
biologically when lines cross in an interaction plot? What does it mean
...
\textbf{Think-Pair-Share} (\#tps-express-int) Express the nature of the
...
\textbf{Think-Pair-Share} (\#tps-small-older) Another thing we could say
is that the effect of exercise is smaller for older people. How is this
shown in the graph?
...
\textbf{Think-Pair-Share} (\#tps-adding-effects) If the effects of diet
and exercise were additive, what would we expect the effect size to be
if we estimated the effect of diet and exercise on blood pressure in the
same study?

\textbf{Think-Pair-Share} (\#tps-comb-non-add) What would we expect the
effect size of combined diet and exercise to be if the effects were
non-additive? (Hint: this is a bit of a trick question.)
...
\textbf{Think-Pair-Share} (\#tps-other-int-examples) Can you think of
any other examples of interactions in biology or medicine?
...
\textbf{Think-Pair-Share} (\#tps-without-interaction) What would a
linear model without an interaction term be?
...
\textbf{Think-Pair-Share} (\#tps-with-interaction) What might the model
look like if we wanted to include an interaction between diet and
exercise?
...
\textbf{Think-Pair-Share} (\#tps-sketch-interaction) Make sketches of
the possible relationships between diet, exercise and blood pressure.
Make a sketch compatible with \(\beta_3 = 0\). Make a sketch compatible
with \(\beta_3 \neq 0\).
...
\textbf{Think-Pair-Share} (\#tps-interaction-null) What is the null
hypothesis in this case, verbally, and in terms of the coefficients of
the model?
...
Of course, we check the model diagnostics before we interpret the
results:
...
\textbf{Think-Pair-Share} (\#tps-two-equations) Write two equations, one
for each of the two diets. They would look something like this:
\(y_i = 0.1 - 0.1 x_{i}^{(1)}\), but will have other numbers.
...
\textbf{Think--Pair--Share (\#a\_interaction\_vs\_main\_effect)} How
does an interaction change the meaning of a ``main effect''? Can a main
effect be misleading when an interaction is present?
...
We can check the model diagnostics:
...
\textbf{Think-Pair-Share} (\#tps-two-way-report) Write a sentence
reporting the results of the two-way ANOVA. Focus on the biological
interpretation of the results, rather than the statistical details.
Include statistics in parentheses, in support of your statements.
...
\begin{tcolorbox}[enhanced jigsaw, leftrule=.75mm, colback=white, title=\textcolor{quarto-callout-caution-color}{\faFire}\hspace{0.5em}{Caution}, breakable, rightrule=.15mm, colframe=quarto-callout-caution-color-frame, opacitybacktitle=0.6, coltitle=black, colbacktitle=quarto-callout-caution-color!10!white, opacityback=0, left=2mm, bottomrule=.15mm, bottomtitle=1mm, toptitle=1mm, arc=.35mm, titlerule=0mm, toprule=.15mm]
...
\tightlist
\item
  A \textbf{continuous response variable}
\item
...
Linear models are powerful, but these assumptions are often violated in
biological data.

In this chapter we move beyond normal linear models to handle an
important new type of response variable: \textbf{counts}.

We introduce \textbf{Generalized Linear Models (GLMs)}, which extend
linear models by allowing the response variable to follow distributions
...
\textbf{Think--Pair--Share} (\#tps-non-neg-kinds) What kinds of
biological outcomes can you think of that \emph{cannot} be negative or
continuous?

\subsection*{From LM to GLM}\label{from-lm-to-glm}
\addcontentsline{toc}{subsection}{From LM to GLM}

Linear models (\texttt{LM}) describe the \textbf{mean of a response
variable} as a linear function of explanatory variables. For example:

\(y_i = \beta_0 + \beta_1 x_i^{(1)} + \cdots + \beta_p x_i^{(p)} + \epsilon_i\)

where the error term \(\epsilon_i\) is normally distributed with mean 0
and constant variance. That is:

\(\epsilon_i \sim N(0, \sigma^2)\).

This works well when the response variable is continuous and
...
\item
  \textbf{Binary outcomes} (e.g.~alive/dead)
\item
  \textbf{Proportions}
\end{itemize}

In these cases, forcing the data into a linear model often leads to
invalid predictions and misleading inference.

\textbf{Generalized Linear Models (GLMs)} solve this problem by:

\begin{itemize}
\tightlist
\item
  Keeping the familiar \emph{linear predictor}, but
\item
  Allowing different distributions for the response, and
\item
  Linking the predictor to the mean response using a \emph{link
  function}.
\end{itemize}
...
\textbf{Key idea}\\
GLMs are not a replacement for linear models --- they are a
\emph{generalisation} of them. Linear regression is a special case of a
GLM.

\end{tcolorbox}

\textbf{Think--Pair--Share} (\#tps-which-reg-assump) Which assumption of
linear regression do you think is most problematic for count data?

\subsection*{Count data}\label{count-data}
\addcontentsline{toc}{subsection}{Count data}

Count data occur frequently in biology and medicine. Typical examples
include:
...
\tightlist
\item
  They are \textbf{discrete}
\item
  They are \textbf{non-negative}
\item
  Their \textbf{variance often increases with the mean}
\item
  There is no known upper limit to the count.
\end{enumerate}

These properties immediately suggest that standard linear regression may
be inappropriate.

\textbf{Think--Pair--Share} (\#tps-which-props-viol) Which of these
properties is violated if we use a normal distribution for counts?
...
\textbf{Think--Pair--Share} (\#tps-what-suspicious) What features of
this plot might already make you suspicious about using linear
regression?

\subsection*{The wrong analysis}\label{the-wrong-analysis}
\addcontentsline{toc}{subsection}{The wrong analysis}

A common mistake is to analyse count data using linear regression,
treating counts as if they were continuous.
...
The diagnostic plots show clear violations of linear regression
assumptions:
...
The qq-plot looks fine. The scale-location plot shows that variance
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{mod2\_soay\_lm }\OtherTok{\textless{}{-}} \FunctionTok{lm}\NormalTok{(fitness }\SpecialCharTok{\textasciitilde{}}\NormalTok{ body.size }\SpecialCharTok{+} \FunctionTok{I}\NormalTok{(body.size}\SpecialCharTok{\^{}}\DecValTok{2}\NormalTok{), }\AttributeTok{data =}\NormalTok{ soay)}
\FunctionTok{par}\NormalTok{(}\AttributeTok{mfrow =} \FunctionTok{c}\NormalTok{(}\DecValTok{2}\NormalTok{, }\DecValTok{2}\NormalTok{))}
\FunctionTok{plot}\NormalTok{(mod2\_soay\_lm, }\AttributeTok{which =} \DecValTok{1}\SpecialCharTok{:}\DecValTok{4}\NormalTok{, }\AttributeTok{add.smooth =} \ConstantTok{TRUE}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\pandocbounded{\includegraphics[keepaspectratio]{8.1-GLM1-count-data_files/figure-pdf/unnamed-chunk-9-1.pdf}}

Notice that for small body sizes the fitted line goes below 0 on the
y-axis. That is, the moe predicts negative fitness values, which are
biologically impossible.

\begin{tcolorbox}[enhanced jigsaw, leftrule=.75mm, colback=white, title=\textcolor{quarto-callout-caution-color}{\faFire}\hspace{0.5em}{Caution}, breakable, rightrule=.15mm, colframe=quarto-callout-caution-color-frame, opacitybacktitle=0.6, coltitle=black, colbacktitle=quarto-callout-caution-color!10!white, opacityback=0, left=2mm, bottomrule=.15mm, bottomtitle=1mm, toptitle=1mm, arc=.35mm, titlerule=0mm, toprule=.15mm]

We have extrapolated beyond the data range here, largely for
illustrative purposes. In practice we should be very cautious about
extrapolating beyond the range of observed data. This is because the
model may not hold outside the observed range, leading to nonsensical
predictions.
...
\textbf{Think--Pair--Share (\#a\_why\_poisson)} Why is a normal
distribution a poor choice for count data? Which assumptions can be
expected to fail?
...
To deal with count data, we need a different probability (error) model.
A common probability model for counts is the \textbf{Poisson
...
respects discreteness, non-negativity, and increasing variance.
...
\pandocbounded{\includegraphics[keepaspectratio]{8.1-GLM1-count-data_files/figure-pdf/unnamed-chunk-10-1.pdf}}
...
\textbf{Think--Pair--Share} (\#a\_mean\_variance\_link) Why does
increasing variance with the mean cause problems for linear regression?

\subsection*{Generalized Linear Models
(GLMs)}\label{generalized-linear-models-glms}
\addcontentsline{toc}{subsection}{Generalized Linear Models (GLMs)}
...
\item
  \textbf{Link function}.
\item
  \textbf{Probability distribution (family)}.
\end{enumerate}

\begin{tcolorbox}[enhanced jigsaw, leftrule=.75mm, colback=white, title=\textcolor{quarto-callout-note-color}{\faInfo}\hspace{0.5em}{Note}, breakable, rightrule=.15mm, colframe=quarto-callout-note-color-frame, opacitybacktitle=0.6, coltitle=black, colbacktitle=quarto-callout-note-color!10!white, opacityback=0, left=2mm, bottomrule=.15mm, bottomtitle=1mm, toptitle=1mm, arc=.35mm, titlerule=0mm, toprule=.15mm]

If you choose a normal family and an identity link, a GLM is
mathematically identical to the linear model you fit using
\texttt{lm()}.

\end{tcolorbox}

\subsubsection*{The linear predictor}\label{the-linear-predictor}
\addcontentsline{toc}{subsubsection}{The linear predictor}

The linear predictor is the same as in linear regression:

\(\eta_i = \beta_0 + \beta_1 x_i^{(1)} + \cdots + \beta_p x_i^{(p)}\)

It is just the linear combination of predictors and coefficients.
Nothing new here.

\subsubsection*{The link function}\label{the-link-function}
\addcontentsline{toc}{subsubsection}{The link function}

In the linear models we already used (e.g., for linear regression) the
link function is the \textbf{identity link}:

\(E(y_i) = \eta_i\)

Here, \(E(y_i)\) refers to the expected value (mean) of many
hypothetical observations with the same predictors, not the single
observed value \(y_i\).

\emph{Identity link}: the expected value of the response \(E(y_i)\)
equals the linear predictor \(\eta_i\).
...
The solution is a different \textbf{link function}. For Poisson
regression, the standard choice is the \textbf{log link}. It relates the
linear predictor to the expected value of the response as follows:
...
regardless of the values of the predictors. Whatever the value of the
linear predictor \(\eta_i\), the exponential of it \(\exp(\eta_i)\) is
always positive.

A good link function enforces the natural constraints of the data.

\textbf{Think--Pair--Share} (\#tps-what-wrong-id) What would go wrong if
we used an identity link for count data?

\subsubsection*{The probability distribution
(family)}\label{the-probability-distribution-family}
\addcontentsline{toc}{subsubsection}{The probability distribution
(family)}

The final component of a GLM is the \textbf{probability distribution}
(also called the \textbf{family}). This specifies how the response
variable is distributed. In Poisson regression, we assume that the
response variable follows a Poisson distribution. Hence, we say we are
fitting a \textbf{Poisson GLM}.
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{soay\_glm }\OtherTok{\textless{}{-}} \FunctionTok{glm}\NormalTok{(fitness }\SpecialCharTok{\textasciitilde{}}\NormalTok{ body.size, }\AttributeTok{data =}\NormalTok{ soay, }\AttributeTok{family =}\NormalTok{ poisson)}
\end{Highlighting}
\end{Shaded}

Specifying \texttt{family\ =\ poisson} tells R to use the Poisson
distribution with a log link function by default. We could specify the
link explicitly as \texttt{family\ =\ poisson(link\ =\ "log")}, but this
is unnecessary since the log link is the default for the Poisson family.
...
\pandocbounded{\includegraphics[keepaspectratio]{8.1-GLM1-count-data_files/figure-pdf/unnamed-chunk-12-1.pdf}}
...
their interpretation differs. The deviance residuals should
...
concerning, but for now we will focus on the other plots.
...
Coefficients are estimated on the \textbf{log scale}.

\begin{tcolorbox}[enhanced jigsaw, leftrule=.75mm, colback=white, title=\textcolor{quarto-callout-important-color}{\faExclamation}\hspace{0.5em}{Important}, breakable, rightrule=.15mm, colframe=quarto-callout-important-color-frame, opacitybacktitle=0.6, coltitle=black, colbacktitle=quarto-callout-important-color!10!white, opacityback=0, left=2mm, bottomrule=.15mm, bottomtitle=1mm, toptitle=1mm, arc=.35mm, titlerule=0mm, toprule=.15mm]

A one-unit increase in a predictor multiplies the expected count by
\(\exp(\beta)\).

\end{tcolorbox}

\textbf{Think--Pair--Share} (\#tps-what-change-beta) If \(\beta=0.05\)
what change in the expected count would be caused by an increase in body
mass of 1 kg? As well as a numeric value, specify whether it is used
additively or multiplicatively.

To interpret the coefficient for body size (\(\beta_1 = 0.05\)), we
exponentiate it: \(\exp(0.05) \approx 1.65\)

This means that for each additional kilogram of body size, the expected
count of fitness increases by a factor of approximately 1.65 (i.e., a
65\% increase). We use the number multiplicatively because of the log
link function.

To make a prediction of the expected count for a given body size, we
back-transform the linear predictor:
...
model, as well as the change in deviance when adding predictors. We can
use a chi-squared test to assess whether adding predictors significantly
improves model fit.
...
\pandocbounded{\includegraphics[keepaspectratio]{8.1-GLM1-count-data_files/figure-pdf/unnamed-chunk-17-1.pdf}}
...
\tightlist
\item
  Unmeasured explanatory variables. This means important predictors are
  missing from the model, leading to extra variability.
\item
  Individual heterogeneity. This can arise when individuals differ in
  ways not captured by measured predictors, leading to extra variability
  in counts.
\item
...
A simple diagnostic for overdispersion is to compare the residual
deviance to the residual degrees of freedom:
...
\textbf{Think--Pair--Share} (\#tps-why-ind-diff) Why might individuals
differ even after accounting for measured predictors?
...
\textbf{Think--Pair--Share} (\#a\_overdispersion\_consequences) What
happens to standard errors and p-values if overdispersion is ignored?
...
\textbf{Think--Pair--Share} (\#tps-zero-inf-examples) Can you think of
an ecological, biological, or medical process that would generate many
structural zeros?
...
\pandocbounded{\includegraphics[keepaspectratio]{8.1-GLM1-count-data_files/figure-pdf/unnamed-chunk-25-1.pdf}}
...
predictor being held constant, just as in multiple linear regression.

\textbf{Think--Pair--Share} (\#tps-how-vis-poisson) How would you
visualise (make a graph) of the data and the modelled relationships?
Make a sketch of what you could do.
...
\chapter*{Binary data (L9)}\label{binary-data-l9}
\addcontentsline{toc}{chapter}{Binary data (L9)}

\markboth{Binary data (L9)}{Binary data (L9)}
...
But many biological outcomes are not continuous. In the previous chapter
we handled \textbf{counts} with a Poisson GLM. In this chapter we move
to an even more common special case: \textbf{binary outcomes}.
...
\textbf{Which explanatory variables influence the probability}
\(\pi_i = P(y_i = 1)\) \textbf{of the outcome?}

\begin{tcolorbox}[enhanced jigsaw, leftrule=.75mm, colback=white, title=\textcolor{quarto-callout-important-color}{\faExclamation}\hspace{0.5em}{Important}, breakable, rightrule=.15mm, colframe=quarto-callout-important-color-frame, opacitybacktitle=0.6, coltitle=black, colbacktitle=quarto-callout-important-color!10!white, opacityback=0, left=2mm, bottomrule=.15mm, bottomtitle=1mm, toptitle=1mm, arc=.35mm, titlerule=0mm, toprule=.15mm]

\textbf{Bridge from LM to GLM}\\
A GLM keeps the familiar \emph{linear predictor} from linear models,
but: 1) uses a distribution (family) that matches the response type,
and\\
...
\textbf{Think--Pair--Share} (\#tps-binary-examples) Name three outcomes
in biology, ecology, or medicine that are naturally binary (0/1). Why
would a linear model be risky for each?
...
\section*{A warm-up: the Chi-square test for a 2×2
table}\label{a-warm-up-the-chi-square-test-for-a-22-table}
\addcontentsline{toc}{section}{A warm-up: the Chi-square test for a 2×2
table}

\markright{A warm-up: the Chi-square test for a 2×2 table}

Binary outcomes often first appear in a \textbf{contingency table}. For
example:
...
The \(\chi^2\) (chi-squared) test asks whether the \textbf{proportion}
(or frequency) of heart attacks is the same in the two contraception
groups.

\subsection*{A concrete example
dataset}\label{a-concrete-example-dataset}
\addcontentsline{toc}{subsection}{A concrete example dataset}

We will reconstruct the classic 2×2 table from some example and store it
as a dataset:
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{heart\_attack }\OtherTok{\textless{}{-}} \FunctionTok{read\_csv}\NormalTok{(}\StringTok{"datasets/heart\_attack\_2x2.csv"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{head}\NormalTok{(heart\_attack)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{tab }\OtherTok{\textless{}{-}} \FunctionTok{xtabs}\NormalTok{(n }\SpecialCharTok{\textasciitilde{}}\NormalTok{ contraception }\SpecialCharTok{+}\NormalTok{ heart\_attack, }\AttributeTok{data =}\NormalTok{ heart\_attack)}
\NormalTok{tab}
\end{Highlighting}
\end{Shaded}
...
\texttt{chisq.test()} defaults to Yates' continuity correction for 2×2
tables. Here we set \texttt{correct\ =\ FALSE} to match the classic
``hand-calculation'' style example.

\end{tcolorbox}

\textbf{Think--Pair--Share} (\#tps-chisq-meaning) What is the null
hypothesis of this \(\chi^2\) test, in words?

\section*{Quantifying an association: risk and
odds}\label{quantifying-an-association-risk-and-odds}
\addcontentsline{toc}{section}{Quantifying an association: risk and
odds}

\markright{Quantifying an association: risk and odds}
...
The \textbf{log odds ratio} is \(\log(OR)\). It is 0 when \(OR=1\).

\begin{tcolorbox}[enhanced jigsaw, leftrule=.75mm, colback=white, title=\textcolor{quarto-callout-important-color}{\faExclamation}\hspace{0.5em}{Important}, breakable, rightrule=.15mm, colframe=quarto-callout-important-color-frame, opacitybacktitle=0.6, coltitle=black, colbacktitle=quarto-callout-important-color!10!white, opacityback=0, left=2mm, bottomrule=.15mm, bottomtitle=1mm, toptitle=1mm, arc=.35mm, titlerule=0mm, toprule=.15mm]

\textbf{Odds are not probabilities}\\
An odds of 3 means ``3 to 1'', which corresponds to a probability of
\(3/(3+1)=0.75\). Odds and probabilities are linked mathematically, but
they are not the same thing.

\end{tcolorbox}

\textbf{Think--Pair--Share} (\#tps-odds-intuition) Why might odds ratios
be a convenient effect size in a regression model? (This is a difficult
question---think carefully, and Pair!)

\section*{From tables to regression
models}\label{from-tables-to-regression-models}
\addcontentsline{toc}{section}{From tables to regression models}

\markright{From tables to regression models}
...
\tightlist
\item
  they typically compare \textbf{groups} (few categorical predictors),
\item
  they don't easily handle \textbf{continuous predictors} (e.g.~dose),
\item
  they don't naturally generalise to multiple predictors.
\end{itemize}

When \(y\) is binary or binomial, the standard regression approach is
\textbf{logistic regression}, a \textbf{binomial GLM}.

\section*{Working example: beetle mortality and insecticide dose
(aggregated
data)}\label{working-example-beetle-mortality-and-insecticide-dose-aggregated-data}
\addcontentsline{toc}{section}{Working example: beetle mortality and
insecticide dose (aggregated data)}

\markright{Working example: beetle mortality and insecticide dose
(aggregated data)}
...
were killed. This is \textbf{binomial (aggregated)} data.
...
It contains one predictor (Dose) and two pieces of information about the
binary outcome (Number\_killed and Number\_tested). The mortality rate
is also included, and is the proportion killed at each dose
(Number\_killed / Number\_tested).
...
\textbf{Think--Pair--Share} (\#tps-why-lm-wrong) What goes wrong if we
fit a linear regression to a probability (mortality rate)? Name at least
two problems.
...
\pandocbounded{\includegraphics[keepaspectratio]{9.1-GLM2-binary-data_files/figure-pdf/unnamed-chunk-12-1.pdf}}
...
trials''. Use a binomial model when each observation is a 0/1 outcome.

\end{tcolorbox}

\textbf{Think--Pair--Share (\#a\_probability\_not\_linear)} Why is it
dangerous to model probabilities directly using a linear model?

\section*{The probability model: Bernoulli and
binomial}\label{the-probability-model-bernoulli-and-binomial}
\addcontentsline{toc}{section}{The probability model: Bernoulli and
binomial}

\markright{The probability model: Bernoulli and binomial}

\subsection*{Bernoulli (binary) data}\label{bernoulli-binary-data}
\addcontentsline{toc}{subsection}{Bernoulli (binary) data}

For a single 0/1 outcome:

\(Y \sim \text{Bernoulli}(\pi),\quad P(Y=1)=\pi,\quad P(Y=0)=1-\pi\)

In words, this means that the probability of success (Y=1) is \(\pi\),
and the probability of failure (Y=0) is \(1-\pi\). And that this
probability is Bernoulli distributed. Bernoulli is a special case of the
...
\subsection*{Binomial (aggregated) data}\label{binomial-aggregated-data}
\addcontentsline{toc}{subsection}{Binomial (aggregated) data}

For \(k\) successes out of \(n\) trials:

\(Y \sim \text{Binomial}(n,\pi)\)

This means that the number of successes \(Y\) in \(n\) independent
Bernoulli trials, each with success probability \(\pi\), follows a
...
\textbf{Key pattern}\\
For Bernoulli/binomial data, the variance is \emph{determined by the
mean}. This is one reason ``constant variance'' fails for linear models.

\end{tcolorbox}

\textbf{Think--Pair--Share} (\#tps-binom-variance) For fixed \(n\), at
what value of \(\pi\) is \(Var(Y)=n\pi(1-\pi)\) largest? At what values
of \(\pi\) is it smallest?

\section*{Logistic regression: the binomial
GLM}\label{logistic-regression-the-binomial-glm}
\addcontentsline{toc}{section}{Logistic regression: the binomial GLM}

\markright{Logistic regression: the binomial GLM}
...
\setcounter{enumi}{1}
\item
  \textbf{Family}: binomial (Bernoulli is the special case \(n=1\))
\item
  \textbf{Link}: logit
\end{enumerate}
...
The inverse link (back-transformation) is the \textbf{logistic
function}:
...
\textbf{Why the logit link?}\\
It maps probabilities \((0,1)\) to the whole real line
\((-\infty,\infty)\), so the linear predictor can take any value while
\(\pi_i\) stays valid, i.e., {[}0,1{]}.

\end{tcolorbox}

\textbf{Think--Pair--Share} (\#tps-identity-link-binary) What would go
wrong if we used the identity link \(E(y_i)=\eta_i\) for a binary
outcome? And which link function is this, by the way?

\section*{Fitting logistic regression in R (aggregated binomial
data)}\label{fitting-logistic-regression-in-r-aggregated-binomial-data}
\addcontentsline{toc}{section}{Fitting logistic regression in R
(aggregated binomial data)}

\markright{Fitting logistic regression in R (aggregated binomial data)}
...
\pandocbounded{\includegraphics[keepaspectratio]{9.1-GLM2-binary-data_files/figure-pdf/unnamed-chunk-16-1.pdf}}
...
\section*{Interpreting coefficients: log-odds, odds ratios, and
probabilities**}\label{interpreting-coefficients-log-odds-odds-ratios-and-probabilities}
\addcontentsline{toc}{section}{Interpreting coefficients: log-odds, odds
ratios, and probabilities**}

\markright{Interpreting coefficients: log-odds, odds ratios, and
probabilities**}
...
\tightlist
\item
  \(\beta_1\) is a \textbf{log odds ratio} for a one-unit change in the
  linear predictor
\item
  \(\exp(\beta_1)\) is the \textbf{odds ratio} (multiplicative change in
  odds)
\end{itemize}

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{beta1 }\OtherTok{\textless{}{-}} \FunctionTok{coef}\NormalTok{(beetle\_glm)[[}\StringTok{"Dose"}\NormalTok{]]}
\FunctionTok{exp}\NormalTok{(beta1)}
\end{Highlighting}
\end{Shaded}
...
Interpretation: increasing dose by 1 unit multiplies the odds of death
by \(\exp(\beta_1)\).

\begin{tcolorbox}[enhanced jigsaw, leftrule=.75mm, colback=white, title=\textcolor{quarto-callout-note-color}{\faInfo}\hspace{0.5em}{Note}, breakable, rightrule=.15mm, colframe=quarto-callout-note-color-frame, opacitybacktitle=0.6, coltitle=black, colbacktitle=quarto-callout-note-color!10!white, opacityback=0, left=2mm, bottomrule=.15mm, bottomtitle=1mm, toptitle=1mm, arc=.35mm, titlerule=0mm, toprule=.15mm]

\textbf{Odds vs probability}\\
A constant odds ratio does \emph{not} mean a constant change in
probability. The probability change depends on where you start
(e.g.~\(\pi=0.05\) vs \(\pi=0.80\)).

\end{tcolorbox}

\textbf{Think--Pair--Share} (\#tps-or-vs-probability) Why might a
treatment have a ``big'' odds ratio, but only a ``small'' change in
probability in one situation?

\textbf{Think--Pair--Share (\#a\_odds\_vs\_probability)} Why might odds
...
As for count-data GLMs, we use likelihood and deviance. Nested models
can be compared with a \(\chi^2\) test.
...
As with the Poisson GLM, the model is fit using maximum likelihood
estimation, and the deviance is used to assess goodness-of-fit. So we
see a table quite similar to that for the Poisson GLM. In this case the
Dose row shows a huge amount of the deviance is explained by dose, and
so the \(p\)-value is very small.
...
\pandocbounded{\includegraphics[keepaspectratio]{9.1-GLM2-binary-data_files/figure-pdf/unnamed-chunk-20-1.pdf}}
...
feature of logistic regression: the predicted probabilities are always
between 0 and 1. We also see that the standard errors are larger at
intermediate doses, where the slope of the curve is steepest. And the
standard errors are smaller at very low and very high doses, where the
curve flattens out.
...
expects. A quick (rough) check for aggregated binomial data is:
...
In some datasets we observe a single 0/1 outcome per individual, rather
than aggregated counts. The same logistic regression model is used, but:

\begin{itemize}
\tightlist
\item
  simple plots of \(y\) against predictors look uninformative (two
  bands)
\item
...
The ESR outcome is in two variables. \texttt{ESR} is a factor with
levels ``low'' and ``high''. It is also present as a numeric variable
\texttt{y}, coded 0 (low) and 1 (high). The predictors are
\texttt{fibrinogen} and \texttt{globulin}, both continuous.
...
proportion of 0s and 1s at each value of the predictor.
...
\pandocbounded{\includegraphics[keepaspectratio]{9.1-GLM2-binary-data_files/figure-pdf/unnamed-chunk-27-1.pdf}}
...
\addcontentsline{toc}{subsection}{Fit the logistic regression model}
...
\pandocbounded{\includegraphics[keepaspectratio]{9.1-GLM2-binary-data_files/figure-pdf/unnamed-chunk-29-1.pdf}}
...
Sometimes a predictor (or combination of predictors) perfectly predicts
the outcome (all 0s on one side, all 1s on the other). This is called
\textbf{(complete) separation} and can cause extremely large coefficient
...
odds ratios \textbf{and} translate to probabilities at meaningful
predictor values.
...
multiple predictors, different types of predictor (continuous and
categorical), interactions, and so on. But the key ideas are all above.
...
\item
  The key questions are about how predictors influence the probability
  of ``yes''.
\item
...
\item
  With non-aggregated individual-level 0/1 data, plotting and
...
In earlier chapters, we mostly worked with \textbf{one response variable
at a time}. But many biological questions are intrinsically
\textbf{multivariate}:
...
It can also help in cases where we have many predictor variables
...
\textbf{Think--Pair--Share} (\#tps-ordination-why) Name one dataset you
work with (or could imagine working with) that has many response
variables. What would you like to learn from it that a single-response
analysis might miss?
...
\textbf{Think--Pair--Share} (\#tps-size-shape) How would you distinguish
``size change'' from ``shape change'' using these plots?
...
\textbf{Think--Pair--Share (\#a\_why\_ordination)} What information is
lost when you analyze multivariate data one variable at a time? Why
...
\FunctionTok{ggplot}\NormalTok{(scores, }\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ PC1, }\AttributeTok{y =}\NormalTok{ PC2, }\AttributeTok{color =}\NormalTok{ thousand.years)) }\SpecialCharTok{+}
  ...
  \FunctionTok{labs}\NormalTok{(}\AttributeTok{color =} \StringTok{"Thousand}\SpecialCharTok{\textbackslash{}n}\StringTok{years ago"}\NormalTok{) }\SpecialCharTok{+}
  \FunctionTok{theme\_minimal}\NormalTok{()}
\end{Highlighting}
\end{Shaded}
...
\textbf{Think--Pair--Share} (\#tps-pc-meaning) If PC1 is positively
correlated with all four skull measurements, what biological
...
\textbf{Think--Pair--Share} (\#tps-biplot-interpretation) Make a sketch
of how skull size / shape changes along the two PC axis.

\textbf{Think--Pair--Share} (\#a\_size\_vs\_shape) What pattern would
indicate pure size change? What pattern would indicate shape change?
...
\textbf{Think--Pair--Share} (\#tps-when-ordination) In what situations
would ordination methods like PCA be especially helpful? When would they
be less useful? Think about the nature of the data, and, if you like,
...
structures (please note that these plots are only made in the HTML
version of the book, and only there you will be able to interact with
them):
...
\textbf{Think--Pair--Share} (\#tps-ordination-correlation) What output
of the PCA would you expect to differ among these three datasets?
...
\textbf{Think--Pair--Share (\#a\_linear\_vs\_nonlinear\_ordination)}
When might a linear method like PCA be misleading? What does NMDS relax?
...
response is a matrix of multiple variables, and the predictor can be
...
PCA can also be used to reduce multivariate predictors into a few
...
\textbf{Think--Pair--Share} (\#tps-why-mixed) Name one example in
biology where multiple measurements come from the same ``unit''
(individual, plot, lake, strain, batch\ldots). What goes wrong if we
pretend those measurements are independent?
...
\item
  \(x_{ij}\) is the predictor variable for observation \(i\) in group
...
Mixed-model diagnostics can be more involved than \texttt{lm()}, but you
...
\tightlist
\item
  Residuals vs predictor (helpful for nonlinearity with a continuous
  predictor)
\end{enumerate}
...
\textbf{Think--Pair--Share} (\#a\_why\_random\_effects) What problem do
random effects solve that fixed effects alone cannot?

\textbf{Think--Pair--Share} (\#a\_partial\_pooling\_intuition) How does
partial pooling differ from complete pooling or no pooling at all? Hint:
pooling is sharing of information.
...
What is the next step after this course? What are the options to further
improve your skills in data analysis in R? What other types of analyses
could you learn about, and when might you need them. Here are a list of
other types of problem / question that we might have, and types of
analysis that could be relevant.
...
\item
  ...
  predictors and response variable is not linear, you might need to
  learn about non-linear regression techniques. These can estimate the
  parameters of specific non-linear functions, and to assess the
  goodness of fit.
\item
...
\item
  ...
  complex, non-linear relationships between predictors and response
  variables while maintaining some interpretability, you might need to
  learn about GAMs. These models use smooth functions to capture
...
\item
  ...
  models. Variables can be both predictors and responses at the same
  time.
\item
...
\item
  ...
  outcomes (e.g., time until failure, time until death), you might need
  to learn about survival analysis techniques such as Kaplan-Meier
  estimation, Cox proportional hazards models, and parametric survival
  models.
\item
...
questions, data characteristics, and goals. Continuing to build your
skills in R and statistical analysis will open up many opportunities for
