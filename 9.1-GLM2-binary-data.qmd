...
suppressMessages(library(HSAUR3))
...
So far in BIO144, we have focused on **linear models** fitted using
`lm()`. These models assume:
...
we handled **counts** with a Poisson GLM. In this chapter we move to an
even more common special case: **binary outcomes**.
...
**Think–Pair–Share (#binary-examples)**  
Name three outcomes in biology, ecology, or medicine that are naturally
binary (0/1). Why would a linear model be risky for each?
...
Binary outcomes often first appear in a **contingency table**. For
example:
...
write.csv(heart_attack, here("datasets", "heart_attack_2x2.csv"),
          row.names = FALSE)
...
**Think–Pair–Share (#chisq-meaning)** What is the null hypothesis of this $\chi^2$ test, in words?
...
**Think–Pair–Share (#odds-intuition)**  
Why might odds ratios be a convenient effect size in a regression model? (This is a difficult question—think carefully, and Pair!)
...
```{r}
data(beetle)
write.csv(beetle, here("datasets", "beetle_mortality.csv"),
          row.names = FALSE)
```
...
binary outcome (Number_killed and Number_tested). The mortality rate is also included, and is the proportion killed at each dose (Number_killed / Number_tested).
...
**Think–Pair–Share (#why-lm-wrong)**  
What goes wrong if we fit a linear regression to a probability (mortality
rate)? Name at least two problems.
...
::: {.callout-caution}
**Poisson vs binomial in one sentence**  
...
Use a binomial model when each observation is a 0/1 outcome.
:::
...
**Think–Pair–Share (#binom-variance)**  
For fixed $n$, at what value of $\pi$ is $Var(Y)=n\pi(1-\pi)$
largest? At what values of $\pi$ is it smallest?
...
**Think–Pair–Share (#identity-link-binary)**  
What would go wrong if we used the identity link $E(y_i)=\eta_i$ for a
binary outcome? And which link function is this, by the way?
...
**Think–Pair–Share (#or-vs-probability)**  
Why might a treatment have a "big" odds ratio, but only a “small” change
in probability in one situation?
...
write.csv(plasma, here("datasets", "plasma_esr_original_HSAUR3.csv"),
          row.names = FALSE)
```
...
**(complete) separation** and can cause extremely large coefficient
...
odds ratios **and** translate to probabilities at meaningful predictor
values.
