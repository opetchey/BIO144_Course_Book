...
**Think, Pair, Share (#two-shape)** Graphically, a linear regression with one explanatory variable is a line. What is a geometric representation of linear regression with two explanatory variables?
...
**Think, Pair, Share (#assump-match)** Review how we can check these assumptions in the simple linear regression model. Match the following to the assumptions above:
...
**Think, Pair, Share** (#variable-importance) How might we assess how important is each of the explanatory variables, and how important they are relative to each other?
...
**Think, Pair, Share** (#exercise-age) How many minutes of exercise per week would we need to add to our fitness schedule to compensate for the blood pressure increase associated with one year of age?
...
p1 <- ggplot(sim_results, aes(x = factor(correlation), y = beta1_hat)) +
  geom_point(color = "blue", alpha = 0.3, height = 0) +
