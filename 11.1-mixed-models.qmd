...
[**Think–Pair–Share** (#tps-why-mixed) Name one example in biology where multiple measurements come from the same "unit" (individual, plot, lake, strain, batch…). What goes wrong if we pretend those measurements are independent?]{.mark}
...
[**Think–Pair–Share** (#a_why_random_effects) What problem do random effects solve that fixed effects alone cannot?]{.mark}

[**Think–Pair–Share** (#a_partial_pooling_intuition) How does partial pooling differ from complete pooling or no pooling at all? Hint: pooling is sharing of information.]{.mark}
