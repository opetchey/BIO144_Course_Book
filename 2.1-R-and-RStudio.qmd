...
## Getting R and RStudio {#sec-using-r-and-rstudio}
...
## Getting to know the RStudio IDE {#sec-getting-to-know-the-rstudio-ide}
...
## Getting to know R {#sec-getting-to-know-r}
...
## R Version and add-on package versions {#sec-r-versions}
...
#| eval: false
library(readr)
my_data <- read_csv("data/my_data_file.csv")
```
...
## Viewing the data {#sec-viewing-data}
...
```{r}
set.seed(123)
my_data <- tibble(name = paste0("Person_", sprintf("%03d", 1:100)),
...
```{r}
ggplot(my_data2, aes(x = education_level, y = age)) +
  geom_point()
```
...
```{r}
ggplot(my_data, aes(x = factor(age_group), y = score)) +
  geom_boxplot()
```
...
```{r}
plot1 <- ggplot(my_data, aes(x = age, y = score)) +
  geom_point() +
  labs(
    title = "Scatterplot of Age vs Score",
    x = "Age (years)",
...
```{r}  
plot2 <- ggplot(my_data, aes(x = score)) +
...
```{r}
plot3 <- ggplot(my_data, aes(x = factor(age_group), y = score)) +
  geom_boxplot() +
  labs(
...
### Setting a reference level in a linear model {#sec-ref-level}
...
```{r}
ggplot(my_data, aes(x = treatment, y = response)) +
  geom_point() +
  labs(
...
```{r}
model1 <- lm(response ~ treatment, data = my_data)
summary(model1)
```
...
```{r}
model2 <- lm(response ~ treatment, data = my_data)
summary(model2)
```
