...
write.csv(bp_data, "data/bp_age_exercise.csv")
```


<a href="data/bp_age_exercise.csv" download>
...
```{r}
#| echo = FALSE
bp_data <- read.csv("data/bp_age_exercise.csv")
```
...
Here's the formula for the F-statistic:
$$\frac{MSM}{MSE}$$

where:
* $MSM$ is the mean square of the model.
...
1. If there are strong relationships in our data, a lot of variance will be explained (large $MSM$) and not much will remain unexplained (low $MES_{residual}$). The $F$-statistic will be large.

2. If there are weak relationships in our data, not much variance will be explained (low $MSM$)  and a lot will remain unexplained (large $MES_{residual}$). The F-statistic will be small.
...
write.csv(bp_data_correlated, "data/bp_data_correlated.csv")
```

<a href="data/bp_data_correlated.csv" download>
  Download the dataset.
</a>

```{r}
bp_data_correlated <- read.csv("data/bp_data_correlated.csv")
```
