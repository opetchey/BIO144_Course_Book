...
[**Think-Pair-Share** (#tps-two-shape) Graphically, a linear regression with one explanatory variable is a line. What is a geometric representation of linear regression with two explanatory variables?]{.mark}
...
fig <- plotly::plot_ly(
  ...
  scene = list(
    xaxis = list(title = "Explanatory Variable 1 (x1)"),
    yaxis = list(title = "Explanatory Variable 2 (x2)"),
    zaxis = list(title = "Response Variable (y)"))
)
...
[**Think–Pair–Share (#a_partial_effects)** What does "holding other variables constant" mean here? Is this always biologically plausible?]{.mark}
...
[**Think-Pair-Share** (#tps-variable-importance) How might we assess how important is each of the explanatory variables, and how important they are relative to each other?]{.mark}
...
[**Think-Pair-Share** (#tps-exercise-age) How many minutes of exercise per week would we need to add to our fitness schedule to compensate for the blood pressure increase associated with one year of age?]{.mark}
...
p1 <- ggplot(sim_results, aes(x = factor(correlation), y = beta1_hat)) +
  ...
  labs(
    x = "Correlation between explanatory variables (r)\n(i.e., collinearity)",
    y = "Slope estimate"
  ) 
...
[**Think–Pair–Share** (#a_collinearity_intuition) Why can strong correlation among explanatory variables make estimates unstable even if the model fits the data well?]{.mark}
...
[**Think–Pair–Share** (#a_interpreting_coefficients_conditionally) How does the interpretation of a coefficient change when you add another explanatory variable to the model?]{.mark}
