...
<!---
**Think–Pair–Share** (#tps_a_objects_not_files) When you type `x <- ...`, where does `x` live?
--->
...
First, get some datasets for us to work with. The datasets used in this book are available for download as a zip file here: [course_book_datasets.zip](course_book_datasets.zip). You can download this file and unzip it to get all of the datasets used in the book. The datasets are in CSV format, which can be opened in R or other statistical software.

Put the datasets in a `data`folder in your project folder.
...
people <- tibble(
  id = 1:n_people,
  sex = sample(c("Female", "Male"), n_people, replace = TRUE),
  treatment = sample(c("Placebo", "Drug"), n_people, replace = TRUE),
  exercise = sample(c("No", "Yes"), n_people, replace = TRUE),
  age = round(rnorm(n_people, mean = 50, sd = 10))
)

# Expand to repeated measures
bp_multifactor <- expand_grid(
  people,
  visit = visits
) %>%
  mutate(
    # baseline blood pressure depends on age and sex
    baseline = 110 + 
      0.5 * age +
      if_else(sex == "Male", 5, 0),
    
    # treatment lowers BP over time
    treatment_effect = if_else(treatment == "Drug",
                               -3 * visit,
                               0),
    
    # exercise lowers BP slightly
    exercise_effect = if_else(exercise == "Yes",
                              -2,
                              0),
    
    # random individual variation
    random_intercept = rnorm(n_people)[id],
    
    # simulated systolic BP
    systolic_bp = baseline +
                  treatment_effect +
                  exercise_effect +
                  random_intercept +
                  rnorm(n(), sd = 5)
  ) %>%
  select(id, sex, treatment, exercise, age, visit, systolic_bp)
write_csv(bp_multifactor, "datasets/bp_multifactor.csv")
```
...
```{r}
bp_multifactor <- read_csv("datasets/bp_multifactor.csv")
glimpse(bp_multifactor)
```

First, a simple summarise. We will get the mean blood pressure across all visits and all people:
...
How does this work? The `summarise()` function creates a new data frame with summary statistics. The argument `mean_bp = mean(systolic_bp)` calculates the mean of the `systolic_bp` variable and names it `mean_bp`. (The mean of all the observations is sometimes called the "grand mean".)
...
The `group_by(id)` function adds a grouping structure to the data frame based on the `id` variable. This tells the `summarise()` function to calculate the mean blood pressure for each person (i.e., for each unique value of `id`).
...
The output has the line `# Groups: id [60]`, which indicates that the data frame is now grouped by the `id` variable, and there are 60 unique groups (i.e., 60 people).
...
There are a series of excellent cheat sheets for R and RStudio, and for packages in the *tidyverse*. They're pretty dense and packed with information, but they can be very useful as a quick reference. You can find them here: [RStudio Cheat Sheets](https://posit.co/resources/cheatsheets/).

## Extras

### Joining data frames

In this chapter we have only worked with one data frame at a time. However, in real life you will often have multiple data frames that you need to join together. For example, you might have one data frame with demographic information about people (e.g., age, sex, education level) and another data frame with information about their health outcomes (e.g., blood pressure, cholesterol levels). You can use the `dplyr` package to join these data frames together using functions like `left_join()`, `right_join()`, `inner_join()`, and `full_join()`. We won't cover these during the course, but you can read about them in the *R for Data Science* book -- [Joining data frames](https://r4ds.hadley.nz/joins.html).
