...
```{r}
#| echo = FALSE
write.csv(bp_meatheavy, here("datasets", "bp_meatheavy.csv"), row.names = FALSE)
```
...
```{r}
#| echo = FALSE
write.csv(bp_vegetarian, here("datasets", "bp_vegetarian.csv"), row.names = FALSE)
```
...
```{r}
#| echo = FALSE
write.csv(bp_1cont1cat, here("datasets", "bp_1cont1cat.csv"), row.names = FALSE)
```


**Think-Pair-Share** (#general-diet) Can we say anything general about the effect of diet on blood pressure?
...
**Think-Pair-Share** (#general-exercise) Can we say anything general about the effect of exercise on blood pressure?
...
```{r}
#| echo = FALSE
write.csv(bp_2cat, here("datasets", "bp_2cat.csv"), row.names = FALSE)
```
...
**Think-Pair-Share** Express the nature of the interaction in words.
...
```{r}
#| echo = FALSE
write.csv(bp_2cont, here("datasets", "bp_2cont.csv"), row.names = FALSE)
```
...
**Think-Pair-Share** Another thing we could say is that the effect of exercise is smaller for older people. How is this shown in the graph?
...
**Think-Pair-Share (#adding-effects)** If the effects of diet and exercise were additive, what would we expect the effect size to be if we estimated the effect of diet and exercise on blood pressure in the same study?

**Think-Pair-Share answer**: What would we expect the effect size of combined diet and exercise to be if the effects were non-additive? (Hint: this is a bit of a trick question.)
...
**Think-Pair-Share** Can you think of any other examples of interactions in biology or medicine?
...
**Think-Pair-Share** (#without-interaction) What would a linear model without an interaction term be?
...
**Think-Pair-Share** (#with-interaction) What might the model look like if we wanted to include an interaction between diet and exercise?
...
**Think-Pair-Share** (#sketch-interaction) Make sketches of the possible relationships between diet, exercise and blood pressure. Make a sketch compatible with $\beta_3 = 0$. Make a sketch compatible with $\beta_3 \neq 0$.
...
**Think-Pair-Share** (#interaction-null) What is the null hypothesis in this case, verbally, and in terms of the coefficients of the model?
...
**Think-Pair-Share** (#two-equations) Write two equations, one for each of the two diets. They would look something like this: $y_i = 0.1 - 0.1 x_{i}^{(1)}$, but will have other numbers.
...
**Think-Pair-Share** (#two-way-report) Write a sentence reporting the results of the two-way ANOVA. Focus on the biological interpretation of the results, rather than the statistical details. Include statistics in parentheses, in support of your statements.
...
```{r}
#| echo = FALSE
write.csv(bp_2cat_3levels, here("datasets", "bp_3cat.csv"))
```
...
plotly::plot_ly(bp_2cont, x = ~mins_per_week, y = ~age, z = ~bp, type = "scatter3d", mode = "markers", color = age)
```
